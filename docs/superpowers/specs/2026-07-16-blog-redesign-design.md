...
| 단계 | 작업 | 완료 기준 |
|------|------|----------|
| 1 | Chirpy 제거 (3장의 삭제 목록) | 빌드 실패는 예상된 상태 |
| 2 | `_tokens.scss` + `default.html` + `main.scss` | 빈 페이지라도 빌드 통과 |
| 3 | 홈 조립 — hero → card-featured → list-item → tag-sidebar | 26편이 목록에 렌더 |
| 4 | `post` 레이아웃 + 본문 타이포 + 코드블록 | 26편 본문 정상 렌더 |
| 5 | `/tags/:tag` 페이지 | 태그 클릭 시 필터 동작 |
| 6 | 다크모드 토큰 + 토글 | 양쪽 모드 정상 |
| 7 | `search.json` + 클라이언트 필터링 | 검색 동작 |
